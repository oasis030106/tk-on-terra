...
const setColorTheme = (nextTheme) => {
  const target = ['dark', 'light'].includes(nextTheme) ? nextTheme : 'dark';
  document.body.classList.toggle('theme-light', target === 'light');
  const vars = target === 'light'
    ? {
        '--text-primary': '#121318',
...
const determineThemeFromImage = (img) => {
  const width = Math.max(1, Math.min(64, img.naturalWidth || img.width || 1));
  const height = Math.max(1, Math.round(((img.naturalHeight || img.height || 1) / (img.naturalWidth || img.width || 1)) * width));
...
const renderThumbnails = (manga, activeIndex) => {
...
const openModal = (mangaIndex, pageIndex) => {
...
const renderGallery = (items) => {
  ...
  items.forEach((item, index) => {
    ...
    node.addEventListener('focus', showPreview);
